// green-night-pi · extensions/header.ts
// Claude Code 风格启动头部（vendor 自 pi-cc-header v1.0.3，MIT © eriiic7z，见 THIRD_PARTY_NOTICES.md）
// 默认配置已适配本主题：logo 绿色（g，跟随 green-night 主色）、标语 "Pi"；
// 配置与 pi-cc-header 完全兼容（settings.json 的 ccHeader key 直接沿用），
// 命令 /hc /hs /hi /hm /hsp /hv /hdf /htg /hcl /hps 均可用。
import {
	CONFIG_DIR_NAME,
	VERSION,
	getAgentDir,
...
const DEFAULT_STATE: CCHeaderState = {
	logoColorKey: "g", // green-night 适配：默认绿色 logo
	versionColored: 1,
	gradientOn: true,
	stripeEnabled: true,
	showPkgSkills: false,
